...
const AddProduct = () => {
  ...
  const { getToken } = useAppContext(); // Chỉ lấy token từ context
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = await getToken(); // Lấy token từ context
      const { data } = await axios.post("/api/product/add", formData, {
        headers: {
          Authorization: `Bearer ${token}`, // Gửi token đi trong header
        },
      });
