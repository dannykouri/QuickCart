...
const ProductList = () => {
  ...
  const [loadingDelete, setLoadingDelete] = useState(false);  // Thêm loadingDelete để xử lý xóa sản phẩm riêng biệt

  const fetchSellerProduct = async () => {
    try {
      const token = await getToken();
      const { data } = await axios.get('/api/product/seler-list', {
  ...
  const handleDelete = async (id) => {
    ...
    try {
        ...
        const token = await getToken();
        const { data } = await axios.delete(`/api/product/${id}/remove/`, {
            headers: {
